...
class TestMain:
    ...
    @patch('sys.argv', ['llm_client.py', 'Test prompt'])
    def test_main_basic_args(self, mock_call_api, capsys):
        ...
        mock_call_api.assert_called_once_with("Test prompt", None, None, False)
        captured = capsys.readouterr()
        assert captured.out == "Test response\n"

    @patch('llm_client.call_llm_api')
    @patch('sys.argv', ['llm_client.py', '--model', 'test-model', 'Test prompt with spaces'])
    ...
    @patch('llm_client.call_llm_api')
    @patch('sys.argv', ['llm_client.py', '--file', 'test.pdf', '--all-pages', 'Analyze this PDF'])
    ...
    def test_main_no_args(self, capsys):
        ...
        with patch('sys.argv', ['llm_client.py']):
    ...
    @patch('llm_client.call_llm_api')
    @patch('sys.argv', ['llm_client.py', '--file', '/nonexistent/file.pdf', 'Test'])
    ...
    @patch('sys.argv', ['llm_client.py', '--help'])
    def test_main_help_flag(self, capsys):
        ...
        captured = capsys.readouterr()
        assert "Call LLM API with a prompt" in captured.out

    @patch('llm_client.call_llm_api')
    @patch('sys.argv', ['llm_client.py', '--file', 'image.jpg', '--model', 'grok-2-vision-1212',
                        'Describe this image'])
    ...
    @patch('llm_client.call_llm_api')
    @patch('sys.argv', ['llm_client.py', 'Test', '--model', 'nonexistent-model'])
    ...
    @patch('llm_client.call_llm_api')
    @patch('sys.argv', ['llm_client.py', 'Multi word prompt with --file flag', '--file', 'doc.txt'])
    ...
    @patch('sys.argv', ['llm_client.py', '--file', 'test.pdf', '--model'])
    ...
    @patch('llm_client.call_llm_api')
    @patch('sys.argv', ['llm_client.py', '', '--file', 'empty.txt'])
